...
def pronoun_resolve(fs_tag):    
    ...
    f3 = open('/location-to-store-output-files/mm.tagged', 'w')
...
fs_tag = open('/location-to-store-output-files/m.tagged', 'r')


f3= pronoun_resolve(fs_tag)


f = open('/location-to-store-output-files/m.txt', 'r')
...
dom = minidom.parse('/location-to-store-output-files/mm.tagged')
...
if "PRP" in B:    
    if any(i > 1 for i in ll) == True:
       #print "Hello"
       A2 = pronoun_reso(b)
       f = open('/location-to-store-output-files/pro_opm.txt', 'w')
       #f = open('./delete_files_iter_defn/pro_opm.txt', 'w')
       f.write(A2)
       f.close()
       #print A2
else:
   ...
   f1 = f.read()
   #print f1
   fm = open('/location-to-store-output-files/pro_opm.txt', 'w')
   #fm = open('./delete_files_iter_defn/pro_opm.txt', 'w')
   fm.write(f1)
