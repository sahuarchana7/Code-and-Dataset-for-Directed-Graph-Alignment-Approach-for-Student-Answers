...
def remove_punc(string):
    ...
    no_punct = ""
    for char in string:
        #if char not in punctuations or char not in punctuations1:
...
if __name__ == '__main__':
    ...
    #profit_matrix = [[1, 2, 3, 4], [4, 3, 2, 1], [3, 6, 9, 12]]
    #profit_matrix = [[1, 3, 3, 8], [2, 6, 2, 9], [3, 9, 1, 7]]                                 ## used for testing the code....less number of nodes as rows

    #profit_matrix = [[1, 2, 3, 0], [3, 6, 9, 0], [3, 2, 1, 0], [8, 9, 7, 0]]


   
    f = open('/location-to-store-output-files/best_filter/profit_matrix.txt', 'r')
    ...
    hungarian = Hungarian(jnew, is_profit_matrix=True)
   
    hungarian.calculate()
    
    f = open('/location-to-store-output-files/hungarian_results.txt', 'w')
    
    f.write(str(hungarian.get_results()))
    f.close()
