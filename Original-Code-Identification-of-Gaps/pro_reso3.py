...
def pronoun_resolve(fs_tag):
    ...
    f3 = open('/location-to-store-output-files/ss.tagged', 'w')
...
fs_tag = open('/location-to-store-output-files/s1.tagged', 'r')
...
f = open('/location-to-store-output-files/s1.txt', 'r')
...
dom = minidom.parse('/location-to-store-output-files/ss.tagged')
...
if "PRP" in B[1:]:
    ...
    if any(i > 1 for i in ll) == True: 
       ...
       A2 = pronoun_reso(b)
       f = open('/location-to-store-output-files/pro_ops.txt', 'w')       
       f.write(A2)
       f.close()
       ...
       f = open('/location-to-store-output-files/s1.txt', 'r')
       f1 = f.read()
       fs = open('/location-to-store-output-files/pro_ops.txt', 'w')
