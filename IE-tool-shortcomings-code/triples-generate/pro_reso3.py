...
def pronoun_resolve(fs_tag):    
    ...
    f3 = open('/location-where-to-store-output-files/ss.tagged', 'w')
...
fs_tag = open('/location-where-to-store-output-files/s1.tagged', 'r')
...
f = open('/location-where-to-store-output-files/s1.txt', 'r')
...
A = [i[0] for i in text_pos]
B = [i[1] for i in text_pos]


dom = minidom.parse('/location-where-to-store-output-files/ss.tagged')
...
if "PRP" in B[1:]:
    ...
    if any(i > 1 for i in ll) == True: 
       ...
       A2 = pronoun_reso(b)
       f = open('/location-where-to-store-output-files/pro_ops.txt', 'w')
       ...
       f = open('/location-where-to-store-output-files/s1.txt', 'r')
       f1 = f.read()
       fs = open('/location-where-to-store-output-files/pro_ops.txt', 'w')
